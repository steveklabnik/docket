...
/// Changelog type determines which section a change appears in when generating changelogs
...
/// Sort field for listing changes
...
pub struct ChangeMetadata {
    ...
    /// If true, this change was explicitly created as an epic (legacy field).
    /// In the unified model, being an epic is derived from having children.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub is_epic: Option<bool>,
    /// Parent change ID (if this is a sub-change).
    /// This field unifies parent_epic into a general parent relationship.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub parent: Option<String>,
    /// Legacy field: parent epic ID. Alias for `parent` for backward compatibility.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub parent_epic: Option<String>,
    /// Persistent working notes (append-only, derived from ScratchpadAppended events)
...
impl Change {
    /// Parse a change from markdown format (used for migration from legacy format)
    pub fn parse(content: &str) -> Result<Self> {
        let parts: Vec<&str> = content.splitn(3, "---").collect();

        if parts.len() < 3 {
            return Err(anyhow!(
                "invalid change file format: expected YAML frontmatter between --- delimiters"
            ));
        }
        ...
        let metadata: ChangeMetadata = serde_yaml::from_str(yaml_content)?;

        Ok(Change { metadata, body })
    }
    ...
    /// Returns true if this change was explicitly created as an epic.
    /// Note: In the unified model, an epic is simply a change with children.
    /// This method checks the legacy is_epic flag for backward compatibility.
    pub fn is_epic(&self) -> bool {
        self.metadata.is_epic.unwrap_or(false)
    }

    /// Returns the parent change ID if this is a sub-change.
    /// Checks both the new `parent` field and legacy `parent_epic` field.
    pub fn parent(&self) -> Option<&str> {
        self.metadata
            .parent
            .as_deref()
            .or(self.metadata.parent_epic.as_deref())
    }

    /// Returns the parent epic ID if this is a child change (legacy alias for parent())
    pub fn parent_epic(&self) -> Option<&str> {
        self.parent()
    }

    /// Returns true if this is a child of another change
    pub fn is_child(&self) -> bool {
        self.parent().is_some()
    }

    /// Returns the scratchpad content
    pub fn scratchpad(&self) -> &str {
        &self.metadata.scratchpad
    }

    /// Returns true if this change has scratchpad notes
    pub fn has_scratchpad(&self) -> bool {
        !self.metadata.scratchpad.is_empty()
    }
    ...
    /// Returns the set of change IDs that this change is blocked by
    pub fn blocked_by(&self) -> &HashSet<String> {
        &self.metadata.blocked_by
    }

    /// Returns true if this change is blocked by another change
...
mod tests {
    ...
    #[test]
    fn change_parse_valid_markdown() {
        let content = r#"---
id: test1
title: Test Change
status: draft
...
---
This is the change body.

## Goal
Fix the thing."#;

        let change = Change::parse(content).unwrap();

        assert_eq!(change.metadata.id, "test1");
        ...
        assert!(change.body.contains("This is the change body"));
        assert!(change.body.contains("## Goal"));
    }

    #[test]
    fn change_parse_invalid_no_frontmatter() {
        let content = "Just some text without frontmatter";
        assert!(Change::parse(content).is_err());
    }

    #[test]
    fn change_parse_invalid_incomplete_frontmatter() {
        let content = "---\nid: test\n---";
...
Body"#;
        ...
        assert_eq!(change.id(), "abc1");
        assert_eq!(change.title(), "Accessor Test");
        assert!(matches!(change.status(), Status::Approved));
